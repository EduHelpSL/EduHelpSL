...
import { getGeminiApiKey } from "./apiService.js"; // Import secure API service
...
export async function initializeGemini(config) {
  let genAI = null;
  ...
  try {
    ...
    if (!geminiApiKey) {
      const errorMessage =
        "Gemini API Key could not be loaded. Chat functionality will be disabled.";
      console.error(errorMessage);
      // Show a persistent error message to the user
      showError("errorChatInit", errorMessage, true);
      isGeminiActive = false;
    } else {
      // Initialize the GoogleGenerativeAI SDK with secure API key
      ...
      geminiModelInstance = genAI.getGenerativeModel({
        ...
        generationConfig: { temperature: 0.5, topP: 0.5 },
      });
    ...
    showError(
      "errorChatInit",
      error.message || "SDK Initialization Failed",
      true
    );
