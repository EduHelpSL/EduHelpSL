...
export async function loadTranslations(lang) {
    ...
    try {
        ...
        console.error(`Load/Parse fail ${lang}:`, error);
        showError('errorLoadTranslations', error.message);
        return {};
    }
}
...
export function applyTranslations() {
    ...
    document.querySelectorAll('[data-lang-key]').forEach(el => {
        ...
        else if (el.tagName === 'META' && el.name === 'description') el.content = tr;
        else if (el.hasAttribute('aria-label') && !el.classList.contains('lang-btn')) el.setAttribute('aria-label', tr);
    ...
    if (state.currentPage === 'chat' && dom.chatMessages && dom.chatMessages.children.length === 0) {
        addChatWelcomeMessage();
    }

    // Update typing indicator text if chat is active
    if (dom.typingIndicator) {
        dom.typingIndicator.dataset.defaultText = t['loadingText'] || 'AI is thinking...';
...
export async function switchLanguage(lang) {
    ...
    if (lang === state.currentLang && Object.keys(state.translations).length > 0) {
        console.log(`Language already ${lang} and translations loaded. Aborting switch.`);
        return;
    }
