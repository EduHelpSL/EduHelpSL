...
export function updateDynamicTitles() {
    ...
    const subjText = (s) => {
        ...
        const fallbackText = s.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase());
    ...
    const resTypeText = (rt) => {
     ...
     const unitText = (uk) => {
        ...
        // For now, assume config is globally accessible or passed.
        // A better way is to pass necessary data or import config.
         // import { config } from "./config.js"; // Add this import if config is not global
         // For now, referencing config directly, assuming it will be available.
         const unitData = window.config?.fakeVideoData?.[vidState.selectedGrade]?.[vidState.selectedSubject]?.[uk]; // Access via window for now, improve later
    ...
    if (dom.libraryResourceTypeTitle) {
        dom.libraryResourceTypeTitle.textContent = `${t['selectResourceType'] || 'Select Type for'} ${gradeText(libState.selectedGrade)}`;
    }
    if (dom.librarySubjectSelectionTitle) {
        dom.librarySubjectSelectionTitle.textContent = `${gradeText(libState.selectedGrade)} - ${resTypeText(libState.selectedResourceType)} - ${t['selectSubject'] || 'Select Subject'}`;
    }
    if (dom.libraryListTitle) {
        let parts = [gradeText(libState.selectedGrade)];
        if (libState.selectedResourceType) parts.push(resTypeText(libState.selectedResourceType));
    ...
    if (dom.videoSubjectSelectionTitle) {
        dom.videoSubjectSelectionTitle.textContent = `${gradeText(vidState.selectedGrade)} - ${t['selectSubject'] || 'Select Subject'}`;
    }
    if (dom.videoUnitSelectionTitle) {
        dom.videoUnitSelectionTitle.textContent = `${gradeText(vidState.selectedGrade)} - ${subjText(vidState.selectedSubject)} - ${t['selectUnit'] || 'Select Unit'}`;
    }
    if (dom.videoListTitle) {
        dom.videoListTitle.textContent = `${gradeText(vidState.selectedGrade)} - ${subjText(vidState.selectedSubject)} - ${unitText(vidState.selectedVideoUnit)}`;
    }

    // Update general view titles that have data-lang-key
    document.querySelectorAll('.view > .view-title[data-lang-key]').forEach(el => {
        // Avoid overwriting dynamically set titles in Library/Videos
        // Also exclude the main home title which is static in HTML
         if (!el.closest('#library') && !el.closest('#videos') && !el.closest('#home')) {
...
export function switchActiveView(sectionElement, targetViewId, isBackNavigation = false) {
    ...
    views.forEach(view => {
        if (view.id === targetViewId) {
            ...
            if (sectionElement.id === 'library') state.libraryState.activeViewId = targetViewId;
            else if (sectionElement.id === 'videos') state.videoState.activeViewId = targetViewId;
...
export function navigateToPage(pageId) {
    ...
    if (!targetPage) { console.error(`Target page #${pageId} not found.`); return; }
    ...
    if (pageId === 'library' || pageId === 'videos') {
        resetSectionView(pageId);
    }

    // Ensure welcome message in chat if navigating TO chat and it's empty
    // This import creates a dependency on chat.js
    if (pageId === 'chat' && dom.chatMessages && dom.chatMessages.children.length === 0) {
