...
function showError(key, fallback, isCritical = false) {
    ...
    if (dom.errorMessage && dom.errorMessageText) {
        ...
        const message = (window.state?.translations && window.state.translations[key]) ? window.state.translations[key] : fallback;
        dom.errorMessageText.textContent = message;
        dom.errorMessage.style.display = 'block';
    } else {
        alert(`Critical Error: ${fallback}`); // Fallback alert
    }
}
...
export function cacheDomElements() {
    ...
    dom = {
        ...
        footerLinks: document.querySelectorAll('.footer-section a[data-footer-link]'),
        ...
        getStartedNavLinks: document.querySelectorAll('.get-started-section .nav-link'),

        // Library Page - General
        libraryPage: document.getElementById('library'),
        libraryGradeSelectionView: document.getElementById('libraryGradeSelectionView'),
        libraryResourceTypeView: document.getElementById('libraryResourceTypeView'),
        librarySubjectSelectionView: document.getElementById('librarySubjectSelectionView'),
        libraryListView: document.getElementById('libraryListView'),

        // Library Page - Main Search/Filters (Grade Selection View)
        libraryMainControls: document.querySelector('#libraryGradeSelectionView .library-main-controls'),
        ...
        libraryResourceTypeGrid: document.getElementById('library-resource-type-grid'),
        libraryResourceTypeTitle: document.getElementById('libraryResourceTypeTitle'),
        librarySubjectGrid: document.getElementById('library-subject-grid'),
        librarySubjectSelectionTitle: document.getElementById('librarySubjectSelectionTitle'),
        ...
        libraryListSearchContainer: document.getElementById('library-list-search-container'), // Container in list view
        ...
        videoSubjectSelectionTitle: document.getElementById('videoSubjectSelectionTitle'),
    ...
    criticalElements.forEach(item => {
        if (!item.el || (item.el instanceof NodeList && item.el.length === 0)) {
            const identifier = item.el?.id || item.el?.className?.split(' ')[0] || item.name;
    ...
    if (missing) {
        showError('errorDOMInit', 'Critical UI elements missing. Please check the console for details.', true);
    } else {
